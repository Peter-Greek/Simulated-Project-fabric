...
import dev.simulated_team.simulated.backport.core.component.CustomData;
...
public record TypewriterSaveKeyToItemPacket(InteractionHand hand, LinkedTypewriterEntries.KeyboardEntry entry) implements CustomPacketPayload {
    ...
    public void handle(final ServerPacketContext context) {
        if (!PacketValidation.validKey(this.entry.glfwKeyCode) || !context.player().getItemInHand(this.hand).is(SimBlocks.LINKED_TYPEWRITER.asItem()) || context.player().isSpectator()) return;
        final ServerPlayer player = context.player();
        final ItemStack item = player.getItemInHand(this.hand);

        CompoundTag currentTag = new CompoundTag();
        if (SimComponents.has(item, DataComponents.BLOCK_ENTITY_DATA)) {
            currentTag = SimComponents.get(item, DataComponents.BLOCK_ENTITY_DATA).copyTag();
        } else {
            currentTag.putString("id", item.getItem().toString());
        }

        // 1.20.5 threads a registry lookup through serialisation; plain NBT ops
        // carry everything a keyboard entry holds.
        final DataResult<Tag> result = LinkedTypewriterEntries.KeyboardEntry.CODEC
