...
public class ClientHandleHandler extends BlockHoldInteraction {
    ...
    public void startHold(final Level level, final Player player, final BlockPos blockPos) {
        ...
        this.desiredRange = (float) Math.min(projected.distance(eyePosition.x, eyePosition.y, eyePosition.z), Math.min(HandleBlockEntity.MAX_HANDLE_RANGE, SimReach.blockInteractionRange(player)));
    ...
    public void deltaRange(final Player player, final float delta) {
        this.desiredRange = (float) Mth.clamp(this.desiredRange + delta, 1, Math.min(SimReach.blockInteractionRange(player), HandleBlockEntity.MAX_HANDLE_RANGE));
    }
