...
import dev.simulated_team.simulated.backport.core.component.DataComponents;
...
public class LinkedTypewriterItem extends BlockItem {
    ...
    public InteractionResultHolder<ItemStack> use(final Level level, final Player player, final InteractionHand usedHand) {
        final BlockHitResult blockHitResult = RaycastHelper.rayTraceRange(level, player, SimReach.blockInteractionRange(player));
    ...
    @Override
    public void appendHoverText(final ItemStack stack, @javax.annotation.Nullable final net.minecraft.world.level.Level context, final List<Component> tooltipComponents, final TooltipFlag tooltipFlag) {
        super.appendHoverText(stack, context, tooltipComponents, tooltipFlag);
        if (SimComponents.has(stack, DataComponents.BLOCK_ENTITY_DATA)) {
            final CompoundTag tag = SimComponents.get(stack, DataComponents.BLOCK_ENTITY_DATA).copyTag();
            if (tag.contains("Keys", CompoundTag.TAG_LIST)) {
                final int keyCount = tag.getList("Keys", CompoundTag.TAG_COMPOUND).size();
                tooltipComponents.add(((net.minecraft.network.chat.MutableComponent) Component.translatable("simulated.linked_typewriter.key_count", keyCount)).withStyle(ChatFormatting.GOLD));
            }
        }
    }
}
