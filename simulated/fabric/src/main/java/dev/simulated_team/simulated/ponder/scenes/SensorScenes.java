...
public class SensorScenes {
    ...
    public static void navigationTable(final SceneBuilder builder, final SceneBuildingUtil util) {
        ...
        // 1.20.5 puts the lodestone target in a component; 1.20.1 keeps it in
        // three top-level stack tags, which is what the compass item reads.
        final CompoundTag lodestoneTag = lodestoneCompass.getOrCreateTag();
        lodestoneTag.put("LodestonePos", NbtUtils.writeBlockPos(BlockPos.ZERO));
        Level.RESOURCE_KEY_CODEC.encodeStart(NbtOps.INSTANCE, Level.OVERWORLD).result()
                .ifPresent(dimension -> lodestoneTag.put("LodestoneDimension", dimension));
    ...
    private static void setVelocitySensor(final SceneBuilder scene, final Selection selection, final float velocity, final int config) {
        scene.world().modifyBlockEntityNBT(selection, VelocitySensorBlockEntity.class, tag -> {
            ...
            tag.putInt("SignedRedstoneStrength", (int) Mth.clamp(15 * velocity / config, 0f, 15f));
        });
    }
