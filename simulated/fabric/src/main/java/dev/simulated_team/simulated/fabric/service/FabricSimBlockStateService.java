...
import dev.simulated_team.simulated.data.fabric.AugerShaftGen;
 ...
 *
 * <p>Identical to upstream's NeoForge implementation; the model generators come
 * from Porting Lib here rather than NeoForge, and their API is the same.
 */
public class FabricSimBlockStateService implements SimBlockStateService {

    @Override
    public <T extends Block> void genericModelBuilder(final DataGenContext<Block, T> ctx,
                                                      final RegistrateBlockstateProvider prov,
                                                      final Function<BlockState, SimBlockStateGen.XYHolder> xyGetter,
    ...
    @Override
    public <P extends AugerShaftBlock> NonNullBiConsumer<DataGenContext<Block, P>, RegistrateBlockstateProvider>
            augerShaftGenerate(final String name, final boolean cog) {
        return AugerShaftGen.generate(name, cog);
    }

    @Override
    public <T extends AbstractDirectionalAxisBlock> void directionalAxisBlock(
            final DataGenContext<Block, T> ctx, final RegistrateBlockstateProvider prov,
        ...
        prov.getVariantBuilder(ctx.getEntry())
                .forAllStates(state -> {
                    ...
                    final int yRot = direction.getAxis().isVertical()
                            ? alongFirst ? 0 : 90
                            : (int) direction.toYRot();

                    final Object model = modelFunc.apply(state, vertical);
                    if (!(model instanceof final ModelFile file)) {
                        throw new IllegalArgumentException("ModelFunc must return a ModelFile");
                    }
