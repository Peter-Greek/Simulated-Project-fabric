...
import dev.simulated_team.simulated.backport.physics.physics.config.FloatingBlockMaterialDataHandler;
...
public class BlockPropertiesTooltip {
    ...
    private static final Component NONE = ((net.minecraft.network.chat.MutableComponent) Component.translatable("simulated.tooltip.mass.none")).withStyle(ChatFormatting.GRAY);
    private static final Component SUPER_LIGHT = ((net.minecraft.network.chat.MutableComponent) Component.translatable("simulated.tooltip.mass.super_light")).withStyle(ChatFormatting.AQUA);
    private static final Component LIGHT = ((net.minecraft.network.chat.MutableComponent) Component.translatable("simulated.tooltip.mass.light")).withStyle(ChatFormatting.GREEN);
    private static final Component HEAVY = ((net.minecraft.network.chat.MutableComponent) Component.translatable("simulated.tooltip.mass.heavy")).withStyle(ChatFormatting.YELLOW);
    private static final Component SUPER_HEAVY = ((net.minecraft.network.chat.MutableComponent) Component.translatable("simulated.tooltip.mass.super_heavy")).withStyle(style -> style.withColor(SimColors.NUH_UH_RED));
    private static final Component ABSURDLY_HEAVY = ((net.minecraft.network.chat.MutableComponent) Component.translatable("simulated.tooltip.mass.absurdly_heavy")).withStyle(style -> style.withColor(SimColors.NUH_UH_RED));
    private static final Component BOUNCY = ((net.minecraft.network.chat.MutableComponent) Component.translatable("simulated.tooltip.bouncy")).withStyle(ChatFormatting.GREEN);
    private static final Component SLIPPERY = ((net.minecraft.network.chat.MutableComponent) Component.translatable("simulated.tooltip.friction.slippery")).withStyle(ChatFormatting.AQUA);
    private static final Component STICKY = ((net.minecraft.network.chat.MutableComponent) Component.translatable("simulated.tooltip.friction.sticky")).withStyle(ChatFormatting.DARK_GREEN);
    private static final Component FRAGILE = ((net.minecraft.network.chat.MutableComponent) Component.translatable("simulated.tooltip.fragile")).withStyle(style -> style.withColor(SimColors.NUH_UH_RED));
    private static final Component AIRTIGHT = ((net.minecraft.network.chat.MutableComponent) Component.translatable("simulated.tooltip.airtight")).withStyle(ChatFormatting.WHITE);
    private static final Component FLOATING = ((net.minecraft.network.chat.MutableComponent) Component.translatable("simulated.tooltip.floating")).withStyle(ChatFormatting.DARK_GREEN);
    ...
    public static void appendTooltip(final ItemStack stack, final TooltipFlag iTooltipFlag, final Player player, final List<Component> itemTooltip) {
        if (stack.getItem() instanceof final BlockItem blockItem) {
            ...
            final BlockStateExtension properties = BlockStateExtension.of(blockItem.getBlock().defaultBlockState());
            final List<Component> toAdd = new ObjectArrayList<>();

            SimRegistries.propertyTooltip().entrySet().stream().sorted(Map.Entry.comparingByValue()).forEach(f -> {
