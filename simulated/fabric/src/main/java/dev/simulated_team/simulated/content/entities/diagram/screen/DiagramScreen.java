...
import dev.simulated_team.simulated.backport.physics.companion.math.BoundingBox3ic;
...
import dev.simulated_team.simulated.backport.veil.api.client.render.VeilLevelPerspectiveRenderer;
import dev.simulated_team.simulated.backport.veil.api.client.render.VeilRenderSystem;
import dev.simulated_team.simulated.backport.veil.api.client.render.framebuffer.AdvancedFbo;
import dev.simulated_team.simulated.backport.veil.api.client.render.post.PostPipeline;
...
//        }).setDiagramTooltip(() -> SimLang.text("Magnify Selection").component()).setIconSwitch(this::isMagnifying);
    ...
    private void renderContents(final SubLevel subLevel, final float partialTicks) {
        ...
        if (this.renderTime >= 20.0f / FPS) {
            ...
            this.renderTime += minecraft.getDeltaFrameTime();
            return;
        }
    ...
    public static void draw(final SubLevel subLevel, final float partialTicks, final Quaternionf localOrientation, final Matrix4f projMatrix, final Vector3d cameraPos, final float inWidth, final float inHeight, final AdvancedFbo fbo, final AdvancedFbo outlineFbo, final AdvancedFbo finalFbo, final float paletteOffset, final float fadeScale, final int lineColor, final int lineShadowColor) {
        ...
        final Pose3dc renderPose = ((ClientSubLevel) subLevel).renderPose(partialTicks);
        final Quaterniond orientation = new Quaterniond(renderPose.orientation()).conjugate();
        orientation.premul(new Quaterniond(localOrientation).conjugate());
        ...
        if (pipeline != null) {
            ...
            PostPipeline.uniform(pipeline, "LineColor").setVector((LINE_COLOR.getRed()) / 255.0f, (LINE_COLOR.getGreen()) / 255.0f, (LINE_COLOR.getBlue()) / 255.0f, 1.0f);
            PostPipeline.uniform(pipeline, "LineShadowColor").setVector((LINE_SHADOW_COLOR.getRed()) / 255.0f, (LINE_SHADOW_COLOR.getGreen()) / 255.0f, (LINE_SHADOW_COLOR.getBlue()) / 255.0f, 1.0f);
            PostPipeline.uniform(pipeline, "InSize").setVector(inWidth, inHeight);
            PostPipeline.uniform(pipeline, "PaletteOffset").setFloat(paletteOffset);
            PostPipeline.uniform(pipeline, "FadeScale").setFloat(fadeScale);
        }
...
//            }
    ...
    public static void renderFBO(final GuiGraphics graphics, final AdvancedFbo fbo, final int width, final int height) {
        // The framebuffer stand-in has no attachments; nothing is drawn.
        final int id = 0;
    ...
    private static void addForceArrowTooltip(final ForceGroup forceGroup, final int forceCount, final double forceMagnitude, final int color, final List<FormattedText> tooltipLines) {
        final LangBuilder forceNameText = SimLang.translate(forceGroup.name()).color(color);
