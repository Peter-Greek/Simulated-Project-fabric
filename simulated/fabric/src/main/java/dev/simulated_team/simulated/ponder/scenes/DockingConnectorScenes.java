...
public class DockingConnectorScenes {
    public static void DockingConnector(final SceneBuilder builder, final SceneBuildingUtil util) {
        ...
        // Fabric measures fluids in droplets rather than NeoForge millibuckets,
        // and Create's own tank here sets its contents directly rather than
        // filling through a handler. Twelve buckets either way.
        scene.world().modifyBlockEntity(new BlockPos(11, 1, 4), FluidTankBlockEntity.class, be ->
                be.getTankInventory().setFluid(new io.github.fabricators_of_create.porting_lib.fluids.FluidStack(
                        Fluids.LAVA, net.fabricmc.fabric.api.transfer.v1.fluid.FluidConstants.BUCKET * 12)));
        scene.idle(10);
