...
public class HoneyGlueClientHandler implements InteractCallback {
    ...
    public void updateHovered() {
        ...
        final Vec3 baseTarget = RaycastHelper.getTraceTarget(player, SimReach.blockInteractionRange(player) * 5, baseOrigin);
    ...
    private boolean checkBBValidity(final Player player, final ItemStack honeyGlueStack, final BlockPos hoveredPos, final boolean showText) {
        ...
        if (this.currentState == State.BINDING) {
            ...
            // The component map is 1.20.5; the stack is what carries the data here.
            ...
            if (HoneyGlueMaxSizing.checkBBMax(bb)) {
                ...
                if (!SimComponents.has(honeyGlueStack, DataComponents.MAX_DAMAGE)) {
    ...
    public BlockHitResult getHitResult() {
        ...
        final ClipContext clipContext = new ClipContext(
                ...
                player.getEyePosition().add(player.getViewVector(SimDistUtil.getPartialTick()).scale(SimReach.blockInteractionRange(player))),
                ClipContext.Block.COLLIDER,
                ClipContext.Fluid.NONE,
                // 1.20.1 takes the entity doing the trace here, not a context.
                player
        );
