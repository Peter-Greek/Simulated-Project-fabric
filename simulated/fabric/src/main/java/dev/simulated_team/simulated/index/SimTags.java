...
public class SimTags {
    ...
    /*
     * Upstream names these the way NeoForge does — {@code forge:stones},
     * {@code forge:slime_balls}, {@code forge:gems/amethyst},
     * {@code forge:nuggets/copper}, {@code forge:dyes/red}. Create's
     * {@code forgeItemTag} only swaps the namespace to {@code c}, so on Fabric
     * those become {@code c:stones} and friends, which nothing on this stack
     * defines: an unknown tag is an empty ingredient, so the recipes that use
     * one can never be crafted, and a tag file that references one fails to
     * load outright.
     *
     * The names below are Fabric's own conventional ones, verified present in
     * the Homestead runtime. Recorded in FABRIC_PORT_PLAN.md.
     */
        public static final TagKey<Item> STONE = AllTags.forgeItemTag("stone");
        public static final TagKey<Item> REDSTONE_DUST = AllTags.forgeItemTag("dusts/redstone");
        public static final TagKey<Item> SLIME_BALLS = AllTags.forgeItemTag("slimeballs");
        public static final TagKey<Item> AMETHYST_SHARDS = AllTags.forgeItemTag("amethyst");
    ...
    public static class Misc {
        // Upstream tags map decoration types so a pack can choose which markers
        // the Navigation Table finds. 1.20.1's decorations are an enum with no
        // registry, so there is nothing to tag; MapNavigationTarget filters by
        // decoration kind instead. Recorded in FABRIC_PORT_PLAN.md.
    ...
    static {
        for (final DyeColor color : DyeColor.values()) {
            DYE_MAP.put(color.getName(), AllTags.forgeItemTag(color.getName() + "_dyes"));
        }
    }
