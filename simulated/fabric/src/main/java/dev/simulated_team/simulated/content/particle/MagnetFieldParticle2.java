...
public class MagnetFieldParticle2 extends SimpleAnimatedParticle {
    ...
    public void render(final VertexConsumer buffer, final Camera renderInfo, final float partialTicks) {
        ...
        // 1.20.5 moved the billboard mode onto the particle; on 1.20.1 a
        // TextureSheetParticle always faces the camera, which is the mode this
        // particle asks for anyway.
        ...
        // renderRotatedQuad is private on 1.20.1; the quad is emitted directly.
        this.renderQuad(buffer, quaternionf, x + offsetX, y + offsetY, z + offsetZ, partialTicks);
    }
    ...
    /**
     * One camera-facing quad. 1.20.1 keeps {@code renderRotatedQuad} private in
     * {@code SingleQuadParticle}, so the same four vertices are written here.
     */
    private void renderQuad(final VertexConsumer buffer, final Quaternionf rotation,
                            final float x, final float y, final float z, final float partialTicks) {
        ...
        buffer.vertex(corners[0].x(), corners[0].y(), corners[0].z()).uv(maxU, maxV)
                .color(this.rCol, this.gCol, this.bCol, this.alpha).uv2(light).endVertex();
        buffer.vertex(corners[1].x(), corners[1].y(), corners[1].z()).uv(maxU, minV)
                .color(this.rCol, this.gCol, this.bCol, this.alpha).uv2(light).endVertex();
        buffer.vertex(corners[2].x(), corners[2].y(), corners[2].z()).uv(minU, minV)
                .color(this.rCol, this.gCol, this.bCol, this.alpha).uv2(light).endVertex();
        buffer.vertex(corners[3].x(), corners[3].y(), corners[3].z()).uv(minU, maxV)
                .color(this.rCol, this.gCol, this.bCol, this.alpha).uv2(light).endVertex();
    }
