...
public class KeyWidget extends AbstractSimiWidget {
    ...
    protected void renderHover(@NotNull final GuiGraphics pGuiGraphics, final int pMouseX, final int pMouseY, final float pPartialTick) {
        ...
        if (this.keyboardActive) {
            ...
            this.renderTooltipBackground(pGuiGraphics, this.getX(), this.getY() - yOffset - bgHeight,
                    bgWidth, bgHeight);
    ...
    private void renderTooltipBackground(@NotNull final GuiGraphics pGuiGraphics, final int x, final int y, final int w, final int h) {
        final SimGUITextures bg = SimGUITextures.LINKED_TYPEWRITER_TOOLTIP_BACKGROUND;
        // 1.20.1 takes the texture size after the region; the tooltip sheet is 256 square.
        pGuiGraphics.blit(bg.location, x, y, 0, 0, w, h, 256, 256);
    }
}