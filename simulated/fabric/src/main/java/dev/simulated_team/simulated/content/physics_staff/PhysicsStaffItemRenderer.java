...
import dev.simulated_team.simulated.backport.veil.Veil;
...
public class PhysicsStaffItemRenderer extends CustomRenderedItemModelRenderer {
    ...
    protected void render(final ItemStack stack, final CustomRenderedItemModel model, final PartialItemModelRenderer renderer, final ItemDisplayContext context, final PoseStack ms,
        ...
        // Upstream asks Iris directly whether a shader pack is in use. Linking
        // against Iris would make this class fail to load without it, so the
        // question is asked through Veil's flag alone: with Iris present the
        // staff takes its shader path, which is the case the check exists for.
        ...
        if (context.firstPerson()) {
            ...
            m.m30(0).m31(0).m32(0);
            m.invert();
            // 1.20.1's PoseStack multiplies by a matrix through its pose, and
            // the rotation is applied to the matrix first.
            m.rotate(new org.joml.Quaternionf(clientHandler.lastCubeOrientation));
            ms.last().pose().mul(m);
        }
