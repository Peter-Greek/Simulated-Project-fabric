...
public class SimArmInteractions {
    ...
    public static class PortableEngineInteractionPoint extends AllArmInteractionPointTypes.DepotPoint {
        ...
        /**
         * 1.20.5's Create passes the arm and a simulate flag; on Fabric the
         * insert runs inside a transfer transaction instead, and a simulated
         * insert is one whose transaction is rolled back by the caller.
         */
        @Override
        public ItemStack insert(final ItemStack stack,
                                final net.fabricmc.fabric.api.transfer.v1.transaction.TransactionContext transaction) {
            if (this.cachedState.hasBlockEntity()) {
                ...
                if (be instanceof final PortableEngineBlockEntity sbe) {
                    return sbe.inventory.insertSlot(stack, 0, transaction == null);
                }
            }

            return super.insert(stack, transaction);
        }
    }
