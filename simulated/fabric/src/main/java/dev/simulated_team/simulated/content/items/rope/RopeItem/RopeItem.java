...
public class RopeItem extends Item {
    ...
    public InteractionResult useOn(final UseOnContext context) {
        ...
        if (player != null && player.isShiftKeyDown()) {
            SimComponents.remove(heldStack, SimDataComponents.ROPE_FIRST_CONNECTION);
            return InteractionResult.SUCCESS;
        }

        if (validLocation) {
            if (SimComponents.has(heldStack, SimDataComponents.ROPE_FIRST_CONNECTION)) {

                if (!level.isClientSide) {
                    if (!this.attachRope(level, SimComponents.get(heldStack, SimDataComponents.ROPE_FIRST_CONNECTION), clickedPos, !player.getAbilities().instabuild)) {
                        // failure to connect
                        SimComponents.remove(heldStack, SimDataComponents.ROPE_FIRST_CONNECTION);
                        return InteractionResult.SUCCESS;
                    } else {
            ...
            SimComponents.set(heldStack, SimDataComponents.ROPE_FIRST_CONNECTION,  clickedPos);
            return InteractionResult.SUCCESS;
        }
