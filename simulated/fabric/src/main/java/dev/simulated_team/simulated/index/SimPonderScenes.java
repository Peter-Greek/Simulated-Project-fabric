...
public class SimPonderScenes {
    public static void register(final PonderSceneRegistrationHelper<ResourceLocation> registry) {
        final PonderSceneRegistrationHelper<ItemProviderEntry<?>> helper = registry.withKeyFunction(com.tterrag.registrate.util.entry.RegistryEntry::getId);
    ...
    /**
     * A ponder scene can name a vanilla item as its icon. Upstream wraps it in a
     * NeoForge {@code DeferredHolder}; 1.20.1 Registrate takes the registry
     * object directly, and the item is already registered by the time a scene
     * is built.
     */
    private static ItemProviderEntry<Item> vanillaItemProvider(final String id) {
        ...
        return new ItemProviderEntry<>(
                ...
                com.tterrag.registrate.fabric.RegistryObject.of(
                        location, net.minecraft.core.registries.BuiltInRegistries.ITEM)
        );
    }
}