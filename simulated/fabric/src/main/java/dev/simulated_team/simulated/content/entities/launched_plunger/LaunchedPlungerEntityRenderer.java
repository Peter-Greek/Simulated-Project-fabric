...
import dev.simulated_team.simulated.util.SimRenderMath;
...
public class LaunchedPlungerEntityRenderer extends EntityRenderer<LaunchedPlungerEntity> {
    ...
    public static Vec3 getFirstPersonFocusPos(final float pt) {
        ...
        final Matrix4f actualProjMat = gameRenderer.getProjectionMatrix(SimRenderMath.fieldOfView());
        ...
        final double fov = SimRenderMath.fieldOfView();
    ...
    public void render(final LaunchedPlungerEntity entity, final float f, final float pt, final PoseStack poseStack, final MultiBufferSource multiBufferSource, final int light) {
        ...
        if (entity.isPlunged()) {
            ...
            selfNormal = SimMathUtils.viewVector(-Mth.lerp(pt, entity.xRotO, entity.getXRot()),-Mth.lerp(pt, entity.yRotO, entity.getYRot())).reverse();
            perpendicularNormal = SimMathUtils.viewVector(-Mth.lerp(pt, entity.xRotO, entity.getXRot()),-Mth.lerp(pt, entity.yRotO, entity.getYRot())-90).reverse();
        }
        ...
        if (other != null) {
            ...
            if (other.isPlunged()) {
                ...
                otherNormal = SimMathUtils.viewVector(-Mth.lerp(pt, other.xRotO, other.getXRot()),-Mth.lerp(pt, other.yRotO, other.getYRot())).reverse();
            }
            ...
            if (entity.getData(LaunchedPlungerEntity.OTHER_PLUNGER).isEmpty() && owner == Minecraft.getInstance().player && Minecraft.getInstance().options.getCameraType().isFirstPerson()) {
                ...
                if (owner instanceof final AbstractClientPlayer player && entity.getData(LaunchedPlungerEntity.OTHER_PLUNGER).isEmpty()) {
                    ...
                    final float bodyDifference = Math.abs(headYDirection - Mth.rotLerp(pt, player.yBodyRotO, player.yBodyRot)) / 50f;
                    ...
                    headYDirection = Mth.lerp(lookDelta, headYDirection, Mth.rotLerp(pt, player.yBodyRotO, player.yBodyRot));
                    final Vec3 viewDirection = SimMathUtils.viewVector(headXDirection, headYDirection);
                    final Vec3 handDirection = SimMathUtils.viewVector(0, headYDirection + 90.0f);
