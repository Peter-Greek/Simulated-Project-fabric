...
public class DiagramStickyNote extends DiagramButton {
    ...
    public void populateFBO(final float partialTicks) {
        if (this.renderTime >= 20.0f / DiagramScreen.FPS) {
            ...
            this.renderTime += Minecraft.getInstance().getDeltaFrameTime();
            return;
        }
        ...
        final BoundingBox3dc scopeBounds = this.parent.config.getNoteConfigs().getNoteScope();
        float radius = (float) Math.max(Math.max(scopeBounds.sizeX(), scopeBounds.sizeY()), scopeBounds.sizeZ()) + 1;
        radius *= 0.55F;
