...
public class SimMathUtils {
    ...
    /**
     * Upstream reaches for {@code OrientedBoundingBox3d.UP}, which is Sable's
     * name for the world up axis. Sable is not on this stack until V2, and the
     * constant is only ever used as a dot-product reference, so it is spelled
     * out here rather than pulling the physics engine in.
     */
    private static final Vector3d SABLE_UP = new Vector3d(0.0, 1.0, 0.0);
    ...
    public static double getClosestYaw(final Quaterniond orientation) {
        final double d = SABLE_UP.dot(new Vector3d(orientation.x(), orientation.y(), orientation.z()));
    ...
    /**
     * The direction an entity is looking, from its pitch and yaw.
     *
     * <p>{@code Entity#calculateViewVector} is protected, and several renderers
     * need it for an entity that is not {@code this}. The arithmetic is vanilla's,
     * copied rather than reached for through a mixin.
     */
    public static Vec3 viewVector(final float pitch, final float yaw) {
        final float pitchRadians = pitch * ((float) Math.PI / 180.0F);
        final float negativeYawRadians = -yaw * ((float) Math.PI / 180.0F);
        final float cosYaw = Mth.cos(negativeYawRadians);
        final float sinYaw = Mth.sin(negativeYawRadians);
        final float cosPitch = Mth.cos(pitchRadians);
        final float sinPitch = Mth.sin(pitchRadians);
